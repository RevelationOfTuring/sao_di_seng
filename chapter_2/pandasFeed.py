# 使用pd中的DataFrame来加载数据。
# 本文件为从csv中读取数据称为DataFrame，在利用该pandas的DataFrame对象加载成PandasData对象。
...
data = bt.feeds.PandasData(
    dataname=dataframe,
    datetime=2,  # 使用索引列作日期列。如果设置为datatime=None，表示将dataframe的索引列作为PandasData中datetime的数据
    open=3,  # 开盘价所在列
    high=4,  # 最高价所在列
    low=5,  # 最低价所在列
    close=6,  # 收盘价价所在列
