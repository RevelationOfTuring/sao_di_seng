# 如果是5日均线，前4根bar会执行prenext()，第5根bar会执行nextstart()，第6根bar开始执行next()
# 策略类的方法调用：init() 1次 -> start() 1次 -> prenext()多次 -> startnext() 1次 -> next() 多次
# 注：在一个bar上，先执行 notify_order()(上一个next中下的order)—> notify_trade()(受前面的notify_order()的影响) -> next()
